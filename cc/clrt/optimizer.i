...
%template(region_t)     std::pair<std::vector<unsigned int>,
                                  float>;
%template(possible)     std::map<unsigned int, std::vector<unsigned int>>;
%template(pos)          std::map<unsigned int, std::pair<float,float>>;
%template(position)     std::pair<float,float>;
%include "optimizer.h"
