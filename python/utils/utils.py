...
def comm_range(sensor, gateway):
    """Calculate distance between sensor and gateway
        returns true if sensor is in range of gateway"""
    if (gateway.pos_x < (sensor.pos_x - cf.COVERAGE_RADIUS) or gateway.pos_x > (sensor.pos_x + cf.COVERAGE_RADIUS)):
        return False
    if (gateway.pos_y < (sensor.pos_y - cf.COVERAGE_RADIUS) or gateway.pos_y > (sensor.pos_y + cf.COVERAGE_RADIUS)):
        return False
    return True
...
def plot_traces(traces):
    ...
    plt.xlim(xmin=0)
    plt.ylim(ymin=0)
    plt.grid(True, which='major', color='0.6', linestyle='--')
    plt.show()
